...
const Button: React.FC<ButtonProps> = ({ children, style, disabled = false, onClick }) => {
  const { pending } = useFormStatus();
  let buttonStyle: string;

  switch (style) {
    case "primary":
      buttonStyle = `block w-full bg-primary-index hover:bg-primary-hover text-white text-sm font-bold rounded-md px-4 md:px-8 py-3 transition-all duration-300 ease-in-out hover:shadow-md disabled:opacity-50 hover:drop-shadow-xl disabled:cursor-not-allowed  disabled:bg-primary-index/50 disabled:shadow-none disabled:hover:shadow-none disabled:hover:scale-100 disabled:hover:bg-primary-index/50 disabled:hover:bg-primary-index disabled:hover:drop-shadow-none ${
