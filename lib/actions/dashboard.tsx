...
// getUserProfile
export const getUserProfile = async (displayName: string): Promise<UserProfile | null> => {
  try {
...
// submitUserDevLinks
export const submitUserDevLinks = async (devlinksList: Devlink[]): Promise<void> => {
  const currentUser = auth?.currentUser as User | null;
  if (!currentUser) {
    throw new Error("User not found");
  }
...
export const submitUserProfileDatas = async (profileDatas: UserProfileData): Promise<void> => {
  const currentUser = auth?.currentUser as User | null;
  if (!currentUser) {
    throw new Error("User not found");
  }
